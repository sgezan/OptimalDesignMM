#' Improves unequally-replicated design by swapping treatments across blocks
#'
#' \code{unequal.Optimize.Across} optimizes an experimental design by swapping pairs of treatments across blocks
#'
...
#'
#'  Results <-unequal.Optimize.Across(matdf=des1$matdf,n=10,
#'  traceI,criteria="A",Rinv,Ginv,K,plot=TRUE)
#'  attributes(Results)
#'
#' @export
#'
...
unequal.Optimize.Across <- function(matdf,n,traceI,criteria="A",Rinv,Ginv,K,plot=FALSE) {
  newmatdf <- matdf
  trace <- traceI
  mat <- NULL
  ...
  Des <- list()
  newmatdf <- unequal.SwapAcross(matdf = matdf)
  TRACE <- c()
  for (i in 2:n) {
    newmatdf <- unequal.SwapAcross(matdf = newmatdf)
    ...
    if (unequal.NewValue(matdf=newmatdf, criteria, Rinv, Ginv, K) < trace) {
      print(sprintf("Swapping Across blocks : %d", i, "complete\n",
                    sep = ""))
  ...
  print(sprintf("ODE due to swapping treatments across blocks: %f", ODE, "complete\n",
                sep = ""))
