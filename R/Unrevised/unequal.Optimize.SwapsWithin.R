#' Optimize an unequally replicated design by swapping  pairs of treatments  within blocks
#'
#' \code{unequal.Optimize.SwapsWithin} optimizes an experimental design by swapping pairs of treatments within blocks
#'
...
#'
#'  Results <- unequal.Optimize.SwapsWithin(matdf=des1$matdf,
#'  n=10,traceI,criteria="A",Rinv,Ginv,K,plot=TRUE)
#'  attributes(Results)
#'
#'
#' @seealso \code{\link{SwapPair}}, \code{\link{unequal.NewValue}}

unequal.Optimize.SwapsWithin <- function(matdf,n,traceI,criteria="A",Rinv,Ginv,K,plot=FALSE) {
  newmatdf <- matdf
  trace <- traceI
  mat <- NULL
  ...
  print(sprintf("ODE due to swapping treatments within blocks is: %f", ODE, "complete\n",
                sep = ""))
