#' Optimize an RCB Design with fied blocks and treatments effects using a simulated annealing algorithm
#'
#' @import Matrix
#' @import MASS
#' @import nadiv
...
#'
#' res <- VarCov.rcbd.fixedAll(matdf,Tr,Tc,rhox=0.6,rhoy=0.6,h2=0.3,s20=0,criteria="A")
#'
#' attributes(res)
#' traceI=res$traceI; criteria="A"
#' Rinv = as.matrix(res$Rinv); K = as.matrix(res$K); C11 = as.matrix(res$C11)

#' ans <- Optimize_SimAnn_rcbd.fixedAll(matdf,n=10,traceI,criteria="A",Rinv, C11, K,plot=TRUE)
#' attributes(ans)
#'
#'
#' @export
...
#'
Optimize_SimAnn_rcbd.fixedAll<- function(matdf,n=10,traceI,criteria="A",Rinv, C11, K,plot=FALSE) {
  newmatdf <- matdf
  trace <- traceI
  mat <- NULL
  ...
  for (i in 2:n) {
    ...
    if (NewValue.rcbd.fixedAll(matdf=newmatdf, criteria, Rinv, C11, K) > trace)
    {
      dif <- setdiff(NewValue.rcbd.fixedAll(matdf=newmatdf, criteria, Rinv, C11, K),trace)
      Temp[i] <- 1/i
