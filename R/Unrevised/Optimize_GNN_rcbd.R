#' Optimize an RCB Design using a genetic nearest neighbor algorithm
#'
#' \code{Optimize_GNN_rcbd} optimizes an RCB experimental design using a genetic nearest neighbor algorithm
#'
#'
...
#' blocks = 2; rb = 5; cb = 6; Tr = 10; Tc = 6; trt = length(c(1:30))
#' set.seed(100)
...
#' K = res$K
#'
#' ans <- Optimize_GNN_rcbd(matdf=matdf,n=10,traceI=res$OptimScore,
#' criteria="A",Amat,Rinv,Ginv,K,plot=TRUE)
#' attributes(ans)
#'
#' @export
#'
...
Optimize_GNN_rcbd<- function(matdf,n,traceI,criteria="A",Amat, Rinv, Ginv, K,plot=FALSE)
{
  newmatdf <- matdf
  trace <- traceI
  mat <- NULL
